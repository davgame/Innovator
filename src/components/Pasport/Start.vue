  ...
  <div class="w-full flex flex-col items-center lg:mt-16 mt-18">
    ...
    <div class="relative lg:w-[480px] w-[330px] mb-6 align-items justify-center">
      <textarea
        ...
        class="w-full lg:min-w-[480px] min-w-[280px] lg:min-h-[140px] min-h-[125px] max-h-[200px] px-4 py-6 border-2 border-blue-300 rounded-3xl lg:text-lg text-[15px] outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all duration-200 resize-none overflow-hidden"
      ></textarea>

    <!-- Smart Mood toggle -->
    <button
      @click="toggleSmartMood"
      class="cursor-pointer lg:mb-3 mb-3 absolute left-4 bottom-3 px-3 py-2 rounded-[13px] border text-sm flex items-center gap-2 transition-all duration-200"
      :class="{
        // Активное состояние (Smart Mood включён) – синий фон
        'bg-[#EDF3FE] text-[#0066FF] border-blue-500/26 hover:bg-[#E4EDFD]': smartMood,
        // Неактивное состояние – белый фон, при наведении серый
        'bg-white text-gray-700 border-gray-300 hover:bg-[#F3F3F3]/31': !smartMood
      }"
    >
      <div class="relative w-5 h-5">
        <!-- Синяя иконка для активного состояния -->
        <img
          v-if="smartMood"
          src="/src/assets/icons/smart_active.svg"
          class="w-full h-full absolute inset-0"
          alt="Smart Mood Active"
        />
        <!-- Серая иконка для неактивного состояния -->
        <img
          v-else
          src="/src/assets/icons/smart.svg"
          class="w-full h-full absolute inset-0 filter grayscale"
          alt="Smart Mood"
        />
      </div>
      ...
      <button
        ...
        class="absolute right-3 lg:top-1/3 top-[42px] -translate-y-1/2 bg-blue-500
            text-white p-[14px] rounded-[15px] hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"

      >
        <img src="/src/assets/images/search2.svg" class="text-white w-[20px] h-[20px]" alt="">
      </button>
    </div>
  </div>
    ...
    <!-- Ошибка -->
    <div v-if="error" class="lg:w-[480px] w-[330px] align-items justify-center mb-4 p-4 bg-red-50 rounded-xl border border-red-200">
...
const handleInput = () => {
  ...
  // ПРОСТО ОЧИЩАЕМ РЕЗУЛЬТАТЫ ПРИ НОВОМ ВВОДЕ
  suggestions.value = []
  error.value = ''
